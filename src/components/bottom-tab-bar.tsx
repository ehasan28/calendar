...
type ItemProps = Partial<TabTriggerSlotProps> & { icon: keyof typeof Ionicons.glyphMap; label: string };

export function TabItem({ icon, label, isFocused, ...props }: ItemProps) {
  const theme = useTheme();
  const color = isFocused ? theme.accent : theme.textSecondary;
  const base = String(icon).replace('-outline', '');
  const name = (isFocused ? base : `${base}-outline`) as keyof typeof Ionicons.glyphMap;
  return (
